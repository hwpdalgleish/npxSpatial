classdef DataFrame < dynamicprops
    ...
    methods (Access = private)
        ...
        function rowIdx = loc_idx(self,row)
            if isnumeric(row)
                idx = cellfun(@(x) str2num(x),self.data.Row);
                rowIdx = 0*row;
                try
                    for i = 1:numel(row)
                        rowIdx(i) = find(idx==row(i));
                    end
                catch
            ...
            if (islogical(row) && numel(row)~=size(self.data,1)) || (isnumeric(row) && numel(rowIdx) ~= numel(row))
                error('KeyError: Key values do not match index')
            end
        end
    ...
    methods
        ...
        function out = rowfun(self,func,varargin)
            out = rowfun(func, self.data, varargin{:});
        end
        
        function out = varfun(self,func,varargin)
            out = varfun(func, self.data, varargin{:});
        end
...
%             out = rowfun(func, A.data, varargin{:});
%         end
%         
%         function out = varfun(func, A, varargin)
%             out = varfun(func, A.data, varargin{:});
%         end
    end
    ...
    methods
        ...
        function [varargout] = subsref(self, S, varargin)
            ...
            %
            % NB this supports reference/method chaining by setting this
            % up as a recursive function that sequentially processes calls
                 
            varargout{1} = [];
            
            % determine which references to use for this call
            if numel(S)>1
                % method or loc
                if strcmp(S(1).type,'.') && ischar(S(1).subs) && (ismethod(self, S(1).subs) || strcmp(S(1).subs,'loc'))
                    maxIdx = 2;
                % field
                elseif strcmp(S(1).type,'.') && all(ismember(S(1).subs,self.columns))
                    tmp = S(1);
                    S(1) = S(2);
                    S(1).subs = {S(2).subs{1} tmp.subs};
                    S(2) = [];
                    maxIdx = 1;
                % other function
                elseif strcmp(S(1).type,'.') && ischar(S(1).subs) && (exist(S(1).subs,'builtin') || exist(S(1).subs,'file'))
                    maxIdx = 2;
                % reference
                else
                    maxIdx = 1;
            ...
            switch call_type
                case '.'
                    if ischar(var) && ismethod(self, var)
                    ...
                    elseif ischar(var) && ismember(exist(var),[2 5])
                        % check for other function
                        args = S(2).subs;
                        funcHandle = eval(['@' S(1).subs]);
                        dimIdx = find(strcmpi(args,'dim'));
                        if isempty(dimIdx)
                            dim = 1;
                        else
                            dim = args{dimIdx+1};
                            args([dimIdx dimIdx+1]) = [];
                        end
                        switch dim
                            case 1
                                varargout{1} = rowfun(funcHandle,self.data);
                            case 2
                                varargout{1} = varfun(funcHandle,self.data);
                        end
                    elseif ischar(var) && strcmp(var,'loc')
                        % check for loc method
                        S = S(2);
                        % process loc subs to be passed through again as
                        % regular call
                        if numel(S.subs)==1
                            error('RefError: provide row and column references when using loc')
                        end
            ...
            % if further calls to process, do recursive
            if exist('S2','var') && ~any(strcmp(var,'loc'))
                [varargout{1}] = subsref(varargout{1}, S2);
            elseif ~exist('S2','var') && any(strcmp(var,'loc'))
                [varargout{1}] = subsref(self, S);
            elseif exist('S2','var') && any(strcmp(var,'loc'))
                [varargout{1}] = subsref(self, [S S2]);
            end
        end
